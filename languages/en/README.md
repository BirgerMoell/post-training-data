# English
> Generated from the metadata at the top of each dataset page. Run `python3 scripts/build_indexes.py` after changing an entry.

Entries are grouped from the language description recorded on each page. Broad multilingual entries may not enumerate every included language.

| Dataset / product | Training use | State | Recorded coverage |
| --- | --- | --- | --- |
| [AIME / MATH / GSM / IFEval sources](../../datasets/aime-math-gsm-ifeval-sources/README.md) | RLVR / GRPO / verifiable RL | Planned | primarily en |
| [allenai/Dolci-DPO-Model-Response-Pool](../../datasets/allenai-dolci-dpo-model-response-pool/README.md) | Preference optimization | Candidate | primarily en |
| [allenai/Dolci-Instruct-DPO](../../datasets/allenai-dolci-instruct-dpo/README.md) | Preference optimization | Used in completed run | primarily en |
| [allenai/Dolci-Instruct-RL](../../datasets/allenai-dolci-instruct-rl/README.md) | RLVR / GRPO / verifiable RL | Candidate | primarily en |
| [allenai/Dolci-Instruct-SFT](../../datasets/allenai-dolci-instruct-sft/README.md) | Instruction SFT | Used in completed run | primarily en |
| [allenai/Dolci-Instruct-SFT-Tool-Use](../../datasets/allenai-dolci-instruct-sft-tool-use/README.md) | Tool use and agentic training | Planned | primarily en |
| [allenai/Dolci-RL-Zero-Mix-7B](../../datasets/allenai-dolci-rl-zero-mix-7b/README.md) | RLVR / GRPO / verifiable RL | Planned | primarily en |
| [allenai/Dolci-Think-DPO-32B](../../datasets/allenai-dolci-think-dpo-32b/README.md) | Preference optimization | Candidate | primarily en |
| [allenai/Dolci-Think-DPO-7B](../../datasets/allenai-dolci-think-dpo-7b/README.md) | Preference optimization | Candidate | primarily en |
| [allenai/Dolci-Think-RL-7B / 32B](../../datasets/allenai-dolci-think-rl-7b-32b/README.md) | RLVR / GRPO / verifiable RL | Candidate | primarily en |
| [allenai/rlvr-code-data-python-r1-format-filtered](../../datasets/allenai-rlvr-code-data-python-r1-format-filtered/README.md) | RLVR / GRPO / verifiable RL | Planned | code/en |
| [allenai/RLVR-GSM-MATH-IF-Mixed-Constraints](../../datasets/allenai-rlvr-gsm-math-if-mixed-constraints/README.md) | RLVR / GRPO / verifiable RL | Planned | primarily en |
| [allenai/tulu-3-sft-mixture](../../datasets/allenai-tulu-3-sft-mixture/README.md) | Instruction SFT | Used in completed run | en |
| [birgermoell/oellm-eu-tooluse-v1](../../datasets/birgermoell-oellm-eu-tooluse-v1/README.md) | RLVR / GRPO / verifiable RL, Tool use and agentic training | Used in completed run | en |
| [birgermoell/oellm-longctx-tokenized-natural-128k-256k-pilot-v1](../../datasets/birgermoell-oellm-longctx-tokenized-natural-128k-256k-pilot-v1/README.md) | Long-context extension, Continued pretraining | Published / available | en + multilingual |
| [birgermoell/oellm-longctx-tokenized-structured-128k-256k-v1](../../datasets/birgermoell-oellm-longctx-tokenized-structured-128k-256k-v1/README.md) | Long-context extension, Continued pretraining | Published / available | en |
| [birgermoell/oellm-longctx-tokenized-superlong-512k-1m-2m-v2](../../datasets/birgermoell-oellm-longctx-tokenized-superlong-512k-1m-2m-v2/README.md) | Long-context extension, Continued pretraining | Published / available | en,sv,de,fr,es,pl |
| [European official/national exam bundle (23 IDs)](../../datasets/european-official-national-exam-bundle-23-ids/README.md) | Preference optimization, RLVR / GRPO / verifiable RL | Used in completed run | sq,bg,ca,cs,da,en,es,et,eu,it,pl,sk,sv |
| [fusion-synth](../../datasets/fusion-synth/README.md) | Instruction SFT | Used in research run | en + 8 EU languages |
| [glaiveai/glaive-function-calling-v2](../../datasets/glaiveai-glaive-function-calling-v2/README.md) | RLVR / GRPO / verifiable RL, Tool use and agentic training | Used in completed run | en |
| [HuggingFaceTB/smoltalk2 — Preference](../../datasets/huggingfacetb-smoltalk2-preference/README.md) | Preference optimization | Configured / runnable | primarily en |
| [LLM360/TxT360-3efforts — agent/high](../../datasets/llm360-txt360-3efforts-agent-high/README.md) | Tool use and agentic training | Used in research run | primarily en |
| [Nanbeige/ToolMind — GraphSyn](../../datasets/nanbeige-toolmind-graphsyn/README.md) | Tool use and agentic training | Used in research run | primarily en |
| [NousResearch/hermes-function-calling-v1](../../datasets/nousresearch-hermes-function-calling-v1/README.md) | RLVR / GRPO / verifiable RL, Tool use and agentic training | Used in completed run | en |
| [nvidia/ChatQA2-Long-SFT-data](../../datasets/nvidia-chatqa2-long-sft-data/README.md) | Long-context extension | Planned | primarily en |
| [nvidia/HelpSteer3](../../datasets/nvidia-helpsteer3/README.md) | Preference optimization | Candidate | primarily en |
| [nvidia/Llama-Nemotron-Post-Training-Dataset](../../datasets/nvidia-llama-nemotron-post-training-dataset/README.md) | Reasoning SFT | Used in completed run | primarily en |
| [nvidia/Nemotron-Agentic-v1 — tool-calling](../../datasets/nvidia-nemotron-agentic-v1-tool-calling/README.md) | Tool use and agentic training | Used in research run | primarily en |
| [nvidia/Nemotron-Post-Training-Dataset-v2](../../datasets/nvidia-nemotron-post-training-dataset-v2/README.md) | Reasoning SFT | Configured / runnable | primarily en |
| [nvidia/Nemotron-SFT-Agentic-v2 — interactive-agent](../../datasets/nvidia-nemotron-sft-agentic-v2-interactive-agent/README.md) | Tool use and agentic training | Used in research run | primarily en |
| [nvidia/OpenMathInstruct-2](../../datasets/nvidia-openmathinstruct-2/README.md) | Reasoning SFT | Candidate | primarily en |
| [open-thoughts/OpenThoughts-114k](../../datasets/open-thoughts-openthoughts-114k/README.md) | Reasoning SFT | Candidate | primarily en |
| [open-thoughts/OpenThoughts2-1M](../../datasets/open-thoughts-openthoughts2-1m/README.md) | Reasoning SFT | Candidate | primarily en |
| [OpenEuroLLM function-calling mixture (#220)](../../datasets/openeurollm-function-calling-mixture-220/README.md) | Tool use and agentic training | Used in research run | primarily en |
| [openeurollm/common-pile-annotated](../../datasets/openeurollm-common-pile-annotated/README.md) | Data quality, filtering, and decontamination | Candidate | primarily en |
| [openeurollm/Dolci-Instruct-SFT-decontaminated](../../datasets/openeurollm-dolci-instruct-sft-decontaminated/README.md) | Instruction SFT | Published / available | primarily en |
| [openeurollm/dolci-instruct-sft-tokenized](../../datasets/openeurollm-dolci-instruct-sft-tokenized/README.md) | Instruction SFT | Published / available | primarily en |
| [openeurollm/Dolci-Think-SFT-32B-decontaminated](../../datasets/openeurollm-dolci-think-sft-32b-decontaminated/README.md) | Reasoning SFT | Published / available | primarily en |
| [openeurollm/Dolci-Think-SFT-7B-decontaminated](../../datasets/openeurollm-dolci-think-sft-7b-decontaminated/README.md) | Reasoning SFT | Published / available | primarily en |
| [openeurollm/dolci-think-sft-tokenized](../../datasets/openeurollm-dolci-think-sft-tokenized/README.md) | Reasoning SFT | Published / available | primarily en |
| [openeurollm/Nemotron-Post-Training-Dataset-v2-decontaminated](../../datasets/openeurollm-nemotron-post-training-dataset-v2-decontaminated/README.md) | Reasoning SFT | Published / available | primarily en |
| [openeurollm/open-perfectblend-decontaminated](../../datasets/openeurollm-open-perfectblend-decontaminated/README.md) | Instruction SFT | Published / available | primarily en |
| [openeurollm/orca-agentinstruct-1M-v1-decontaminated](../../datasets/openeurollm-orca-agentinstruct-1m-v1-decontaminated/README.md) | Instruction SFT | Published / available | primarily en |
| [openeurollm/smoltalk2-decontaminated](../../datasets/openeurollm-smoltalk2-decontaminated/README.md) | Preference optimization | Published / available | primarily en |
| [qwen35-9b-dpo-parquet](../../datasets/qwen35-9b-dpo-parquet/README.md) | Preference optimization | Used in completed run | en |
| [qwen35-9b-reasoning-sft-parquet](../../datasets/qwen35-9b-reasoning-sft-parquet/README.md) | Reasoning SFT | Used in completed run | en,fi |
| [qwen35-9b-sft-parquet (tulu3-euroblocks-85-15)](../../datasets/qwen35-9b-sft-parquet-tulu3-euroblocks-85-15/README.md) | Instruction SFT | Used in completed run | EU multilingual + en |
| [Salesforce/xlam-function-calling-60k](../../datasets/salesforce-xlam-function-calling-60k/README.md) | RLVR / GRPO / verifiable RL, Tool use and agentic training | Candidate | en |
| [Team-ACE/ToolACE](../../datasets/team-ace-toolace/README.md) | RLVR / GRPO / verifiable RL, Tool use and agentic training | Used in completed run | en |
| [THUDM/LongAlign-10k](../../datasets/thudm-longalign-10k/README.md) | Long-context extension | Candidate | primarily en |
| [TMAX agentic RL reproduction](../../datasets/tmax-agentic-rl-reproduction/README.md) | Tool use and agentic training | Candidate | primarily en |
| [UltraFeedback](../../datasets/ultrafeedback/README.md) | Preference optimization | Candidate | primarily en |
| [Yukang/LongAlpaca-12k](../../datasets/yukang-longalpaca-12k/README.md) | Long-context extension | Candidate | primarily en |
